...
try
{
    ...
    builder.Services.AddAntiforgery(); // Required for @Html.AntiForgeryToken()
    ...
    app.MapPost("/api/bp/calculate", (BPRequest request) =>
    {
        ...
        var bp = new BloodPressure { Systolic = request.Systolic, Diastolic = request.Diastolic };
        return Results.Ok(new { Category = bp.Category.ToString(), Systolic = bp.Systolic, Diastolic = bp.Diastolic });
    });
    ...
    app.MapPost("/api/bmi/calculate", (BMIRequest request) =>
    {
        ...
        var bmi = new BMI { Weight = request.Weight, Height = request.Height };
        return Results.Ok(new { Score = Math.Round(bmi.BMIScore, 2), Category = bmi.Category });
    });
    ...
    Log.Information("Starting web host. Environment: {Env}", app.Environment.EnvironmentName);
    app.Run();
}
catch (Exception ex) { Log.Fatal(ex, "Host terminated unexpectedly"); }
finally { Log.CloseAndFlush(); }

static string GetBPCategoryMessage(BPCategory category) => category switch
{
...
static string GetBMICategoryMessage(string category) => category switch
{
